...
@pytest.mark.parametrize(
    ...
    [
        ((0.0, 1.0), (604.0, 96.0), (628.0, 96.0), (24.0, 0.0)),
        ((0.0, 1.0), (652.0, 96.0), (628.0, 96.0), (-24.0, 0.0)),
        ((1.0, 0.0), (130.0, 148.0), (130.0, 180.0), (0.0, 24.0)),
        ((1.0, 0.0), (130.0, 204.0), (130.0, 180.0), (0.0, -24.0)),
    ],
)
def test_caretaker_push_movement_moves_player_from_caretaker(
    surface_normal: tuple[float, float],
    caretaker_position: tuple[float, float],
    player_position: tuple[float, float],
    ...
    movement = caretaker_push_movement(
        stain,
        caretaker_bounds,
        player_bounds,
    )

    assert movement == pygame.Vector2(expected_movement)
...
def test_caretaker_rounding_target_clears_player_outer_corner(
    contact_position: tuple[float, float],
    surface_normal: tuple[float, float],
    caretaker_position: tuple[float, float],
    player_position: tuple[float, float],
    ...
    target = caretaker_rounding_target(
        stain,
        caretaker_bounds,
        player_bounds,
    )
...
def test_caretaker_sidestep_target_respects_selected_side() -> None:
    stain = WallStain(
        wall_id="wall",
        contact_position=(640.0, 96.0),
        surface_normal=(0.0, 1.0),
    ...
    target = caretaker_sidestep_target(
        ...
        caretaker_bounds,
        player_bounds,
        side=CaretakerSide.NEGATIVE,
    )

    assert target == pygame.Vector2(604.0, 96.0)
